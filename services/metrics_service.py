...
from schemas.metrics import DashboardMetrics, TeamScorersSchema, TopScorerSchema
...
class MetricsService:
    ...
    def get_team_scorers(self) -> list[TeamScorersSchema]:
        cached = retrieve()
        if not cached:
            raise HTTPException(
                status_code=404,
        ...
        return [TeamScorersSchema.model_validate(item) for item in cached["team_scorers"]]
