...
def test_simulator_run_group_stage_structure(client: TestClient):
    ...
    for g in data["groups"]:
        assert g["group"] in "ABCDEFGHIJKL"
    ...
    for g in data["groups"]:
        for s in g["standings"]:
            if s["position"] <= 2:
                ...
                thirds.append(s)
    best_thirds = sorted(thirds, key=lambda x: (x["pts"], x["gd"], x["gf"]), reverse=True)[:8]
    qualified.extend(s["team"] for s in best_thirds)
    assert len(qualified) == 32
    assert len(set(qualified)) == 32
